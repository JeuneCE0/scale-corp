...
export function PulseScreen({socs,reps,allM,ghlData,socBank,hold,clients,onClose}){
 ...
 const aiTicker=useMemo(()=>{const today=new Date().toISOString().slice(0,10);const parts=[];let totalIn=0,totalOut=0;allActS.forEach(s=>{const sn=s?.nom||s?.name||"";const opps=gd[s.id]?.opportunities||[];const newP=opps.filter(o=>!o.status&&(o.dateAdded||o.createdAt||"").slice(0,10)===today).length;const calls=(gd[s.id]?.calendarEvents||[]).filter(e=>(e?.startTime||"").slice(0,10)===today).length;const excl=EXCLUDED_ACCOUNTS[s.id]||[];let socIn=0,socOut=0;(sb[s.id]?.transactions||[]).filter(tx=>{const leg=tx.legs?.[0];if(leg&&excl.includes(leg.account_id))return false;return(tx.created_at||tx.createdAt||"").slice(0,10)===today;}).forEach(tx=>{const amt=pf(tx.legs?.[0]?.amount||tx?.amount);if(amt>0)socIn+=amt;else socOut+=Math.abs(amt);});totalIn+=socIn;totalOut+=socOut;const won=opps.filter(o=>o.status==="won"&&(o.updatedAt||"").slice(0,10)===today).length;const lost=opps.filter(o=>o.status==="lost"&&(o.updatedAt||"").slice(0,10)===today).length;const bits=[];if(newP>0)bits.push(`👤 +${newP} prospect${newP>1?"s":""}`);if(calls>0)bits.push(`📞 ${calls} appel${calls>1?"s":""}`);if(socIn>0)bits.push(`🤑 +${fmt(socIn)}€`);if(socOut>0)bits.push(`🔻 -${fmt(socOut)}€`);if(won>0)bits.push(`✅ ${won} deal gagné`);if(lost>0)bits.push(`❌ ${lost} deal perdu`);if(bits.length>0)parts.push(`[${sn}] ${bits.join(" · ")}`);});const margin=totalIn-totalOut;if(parts.length===0)parts.push("Aucune activité aujourd'hui");if(totalIn>0||totalOut>0)parts.push(`📊 Solde global : ${margin>=0?"+":""}${fmt(margin)}€`);return parts.join("   •   ");},[allActS,gd,sb]);
