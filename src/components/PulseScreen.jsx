...
export function PulseScreen({socs,reps,allM,ghlData,socBank,hold,clients,onClose}){
 ...
 const periodLabel=timeFilter==="1j"?"ce jour":timeFilter==="7j"?"7 derniers jours":timeFilter==="30j"?"30 derniers jours":"ce mois";
 ...
 const socCards=useMemo(()=>actS.map(s=>{const r=gr(reps,s.id,cM);const rp=gr(reps,s.id,prevMVal);const ca=pf(r?.ca);const caP=pf(rp?.ca);const prosp=getProspects(s.id).filter(p=>inRange(p.dateAdded||p.createdAt||"")).length;const st=s.status==="active"?"🟢":s.status==="lancement"?"🟡":"🔴";const pipVal=pf(gd[s.id]?.stats?.pipelineValue);const rangeTxs=(sb[s.id]?.transactions||[]).filter(tx=>inRange(tx.created_at||tx.createdAt||""));const rangePayAmt=rangeTxs.filter(tx=>{const leg=tx.legs?.[0];return leg&&pf(leg.amount)>0;}).reduce((x,tx)=>x+pf(tx.legs?.[0]?.amount),0);const lastAct=(sb[s.id]?.transactions||[])[0]?.createdAt||(gd[s.id]?.ghlClients||[])[0]?.dateAdded||"";const hs=healthScore(s,reps);return{id:s.id,name:s?.nom||s?.name||"",porteur:s?.porteur||"",status:st,statusRaw:s.status,ca,caP,prosp,trend:ca>caP?"↑":ca<caP?"↓":"→",bal:pf(sb[s.id]?.balance),pipVal,lastAct,logoUrl:s?.logoUrl||"",brandColor:s?.brandColor||"",color:s?.color||"#FFAA00",grade:hs.grade,gradeColor:hs.color,mrr:pf(r?.mrr),rangePayAmt};}).sort((a,b)=>b.ca-a.ca),[actS,reps,cM,prevMVal,gd,sb,timeRange]);
