...
import {
  ...
  MilestonesCompact, MobileBottomNav, Modal, PBar, PulseOverview, RiskMatrix, Sect, Sel, Sidebar, SmartAlertsPanel,
...
setLErr("Code incorrect");setShake(true);setTimeout(()=>setShake(false),500);},[pin,socs]);
 ...
 return <div className="glass-bg" style={{display:"flex",minHeight:"100vh",fontFamily:FONT,color:C.t}}>
  ...
  {missedRecap&&<div className="fi" style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)"}} onClick={()=>setMissedRecap(null)}>
   <div onClick={e=>e.stopPropagation()} style={{width:460,maxWidth:"95vw",maxHeight:"80vh",background:C.card,border:`1px solid ${C.brd}`,borderRadius:20,overflow:"hidden",backdropFilter:"blur(20px)"}}>
    <div style={{padding:"20px 24px",borderBottom:`1px solid ${C.brd}`,background:`linear-gradient(135deg,${C.acc}11,transparent)`}}>
     <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div><div style={{fontSize:18,fontWeight:900,fontFamily:FONT_TITLE,color:C.acc}}>👋 Bon retour !</div>
  ...
  {tab===0&&<>
   ...
   <Sect title="Portfolio" sub={`${actS.filter(s=>s.id!=="eco").length} sociétés actives`}>
    ...
    <div style={{width:24,height:24,borderRadius:6,background:s.color+"22",border:`1.5px solid ${s.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:10,color:s.color,overflow:"hidden"}}>{s.logoUrl?<img loading="lazy" src={s.logoUrl} alt={s.nom} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:s.nom[0]}</div>
    <div style={{flex:1,minWidth:0}}>
  ...
  {tab===1&&<>
   ...
   return <div key={s.id} className={`glass-card fu d${Math.min(i+1,8)}`} style={{padding:16,cursor:"pointer"}} onClick={()=>setESoc({...s})}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
     <div style={{width:36,height:36,borderRadius:10,background:s.color+"22",border:`2px solid ${s.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:15,color:s.color,overflow:"hidden"}}>{s.logoUrl?<img loading="lazy" src={s.logoUrl} alt={s.nom} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:s.nom[0]}</div>
  ...
  {tab===4&&(()=>{
   ...
   <div style={{marginTop:12,padding:"12px 14px",background:C.bg,borderRadius:10,border:`1px solid ${C.brd}`}}>
    ...
    {hold.brand?.logoUrl&&<div style={{marginTop:8,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:9,color:C.td}}>Aperçu:</span><div style={{width:36,height:36,borderRadius:8,overflow:"hidden",border:`1px solid ${C.brd}`}}><img loading="lazy" src={hold.brand.logoUrl} alt={hold.brand?.name||"Aperçu du logo"} style={{width:"100%",height:"100%",objectFit:"cover"}}/></div></div>}
   </div>
