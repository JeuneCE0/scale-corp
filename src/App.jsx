...
function AppInner(){
 ...
 useEffect(()=>{(async()=>{try{const[s,r,h,a,j,p,d,g,rv,sb,ok,sy,kk,ch,su,tm,cl,iv]=await Promise.all([sGet("scAs"),sGet("scAr"),sGet("scAh"),sGet("scAa"),sGet("scAj"),sGet("scAp"),sGet("scAd"),sGet("scAg"),sGet("scAv"),sGet("scAb"),sGet("scAo"),sGet("scAy"),sGet("scAk"),sGet("scAc"),sGet("scAu"),sGet("scAt"),sGet("scAcl"),sGet("scAiv")]);
   ...
   try{const tk=localStorage.getItem("sc_auth_token");if(tk){fetch("/api/auth?action=me",{headers:{Authorization:"Bearer "+tk}}).then(r2=>r2.ok?r2.json():null).then(async(u)=>{if(u&&u.id){setAuthUser(u);const meta=u.user_metadata||{};if(meta.role==="admin"){setStoreToken("auth");setCurrentSocId("admin");syncFromSupabase("admin").catch(()=>{});setRole("admin");}else if(meta.role==="client"){const clientSocId="client_"+u.id;setStoreToken("auth");setCurrentSocId(clientSocId);try{await syncFromSupabase(clientSocId);}catch{}setRole("client");}else if(meta.society_id){setStoreToken("auth");setCurrentSocId(meta.society_id);syncFromSupabase(meta.society_id).catch(()=>{});setRole(meta.society_id);}}setAuthRestoring(false);}).catch(()=>{setAuthRestoring(false);});}else{setAuthRestoring(false);}}catch{setAuthRestoring(false);}
   })();},[]);
...
setLErr("Code incorrect");setShake(true);setTimeout(()=>setShake(false),500);},[pin,socs]);
 ...
 if(role==="client"){const clientMeta=authUser?.user_metadata||{};return <div className="glass-bg" style={{minHeight:"100vh",fontFamily:FONT,color:C.t}}>
  ...
  <div style={{maxWidth:1100,margin:"0 auto",padding:"20px 24px 60px"}}>
   ...
   <Suspense fallback={<LazyFallback/>}><SaaSClientPortal authUser={authUser}/></Suspense>
  </div>
 </div>;}
 ...
 if(!role&&route==="#/client")return <><style>{CSS}{POLISH_CSS}</style><Suspense fallback={<LazyFallback/>}><ClientLoginPage brand={saasBrand} onNavigate={routeNav} onLogin={async(user)=>{setAuthUser(user);const meta=user?.user_metadata||{};const rid=meta.role==="admin"?"admin":meta.role==="client"?"client":(meta.society_id||"admin");const socId=meta.role==="client"?"client_"+user.id:rid;setStoreToken("auth");setCurrentSocId(socId);localStorage.setItem("sc_store_token","auth");try{await syncFromSupabase(socId);}catch{}setRole(rid);setRoute("");window.history.replaceState(null,"",window.location.pathname);}}/></Suspense></>;
