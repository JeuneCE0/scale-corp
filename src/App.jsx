...
import {
  ...
  captureReferral, captureReferralFromPath, findReferrerByCode, saveReferralRecord, convertReferral, buildRefCode, getReferralRecords,
} from "./shared.jsx";
...
import {
  ...
  WidgetEmbed, WidgetRenderer, calcSmartAlerts, IntegrationsPanel,
...
function AppInner(){
 ...
 useEffect(()=>{(async()=>{try{const[s,r,h,a,j,p,d,g,rv,sb,ok,sy,kk,ch,su,tm,cl,iv]=await Promise.all([sGet("scAs"),sGet("scAr"),sGet("scAh"),sGet("scAa"),sGet("scAj"),sGet("scAp"),sGet("scAd"),sGet("scAg"),sGet("scAv"),sGet("scAb"),sGet("scAo"),sGet("scAy"),sGet("scAk"),sGet("scAc"),sGet("scAu"),sGet("scAt"),sGet("scAcl"),sGet("scAiv")]);
   ...
   try{const u2=new URLSearchParams(window.location.search);const oauthResult=u2.get("oauth");if(oauthResult){const oauthProvider=u2.get("provider")||"";const oauthSoc=u2.get("society")||"";const oauthMsg=u2.get("msg")||"";if(oauthResult==="success"){showToast(`✅ ${oauthProvider} connecté pour ${oauthSoc}`,"success");}else{showToast(`❌ Erreur connexion ${oauthProvider}: ${oauthMsg}`,"error");}window.history.replaceState({},"",window.location.pathname);}}catch(err){console.warn('[OAuth callback] failed:',err?.message||err);}
   setLoaded(true);
   // Session persistence: check stored auth token
   try{const tk=localStorage.getItem("sc_auth_token");if(tk){fetch("/api/auth?action=me",{headers:{Authorization:"Bearer "+tk}}).then(r2=>r2.ok?r2.json():null).then(u=>{if(u&&u.id){setAuthUser(u);const meta=u.user_metadata||{};if(meta.role==="admin"){setRole("admin");setStoreToken("auth");setCurrentSocId("admin");syncFromSupabase("admin").catch(()=>{});}else if(meta.society_id){setRole(meta.society_id);setStoreToken("auth");setCurrentSocId(meta.society_id);syncFromSupabase(meta.society_id).catch(()=>{});}}}).catch(()=>{});}}catch(err){console.warn('[auth session restore] failed:',err?.message||err);}
   })();},[]);
 const scChannel=useRef(null);
 useEffect(()=>{try{scChannel.current=new BroadcastChannel("scale-corp-sync");scChannel.current.onmessage=async(e)=>{if(e.data?.type==="socs-updated"){try{const sbSocs=await fetchSocietiesFromSB();if(sbSocs&&sbSocs.length>0)setSocs(prev=>{const sbMap=Object.fromEntries(sbSocs.map(x=>[x.id,x]));return prev.map(sc=>sbMap[sc.id]?{...sc,...sbMap[sc.id]}:sc);});}catch(err){console.warn('[BroadcastChannel socs-updated] failed:',err?.message||err);}}if(e.data?.type==="hold-updated"){try{const sbHold=await fetchHoldingFromSB();if(sbHold)setHold(sbHold);}catch(err){console.warn('[BroadcastChannel hold-updated] failed:',err?.message||err);}}};}catch(err){console.warn('[BroadcastChannel init] failed:',err?.message||err);}return()=>{try{scChannel.current?.close();}catch(err){console.warn('[BroadcastChannel close] failed:',err?.message||err);}};},[]);
 const save=useCallback(async(ns,nr,nh)=>{setSaving(true);trackEvent("save",{hasSocs:!!ns,hasReps:!!nr,hasHold:!!nh});try{if(ns!=null){setSocs(ns);await sSet("scAs",ns);await Promise.all((ns||[]).map(s=>sbUpsert('societies',{id:s.id,...s})));try{scChannel.current?.postMessage({type:"socs-updated"});}catch(err){console.warn('[save broadcast socs] failed:',err?.message||err);}}if(nr!=null){setReps(nr);await sSet("scAr",nr);}if(nh!=null){setHold(nh);await sSet("scAh",nh);await sbUpsert('holding',{id:'main',config:nh});try{scChannel.current?.postMessage({type:"hold-updated"});}catch(err){console.warn('[save broadcast hold] failed:',err?.message||err);}}}catch(e){console.warn("save():",e);}setSaving(false);},[]);
 // Periodic refresh from Supabase (every 15s) to sync changes across different browsers/devices
 useEffect(()=>{if(!loaded)return;const iv=setInterval(async()=>{try{const sbSocs=await fetchSocietiesFromSB();if(sbSocs&&sbSocs.length>0){setSocs(prev=>{const sbMap=Object.fromEntries(sbSocs.map(x=>[x.id,x]));const next=prev.map(sc=>sbMap[sc.id]?{...sc,...sbMap[sc.id]}:sc);if(JSON.stringify(next)===JSON.stringify(prev))return prev;return next;});}const sbHold=await fetchHoldingFromSB();if(sbHold)setHold(h=>JSON.stringify(h)===JSON.stringify(sbHold)?h:sbHold);}catch(err){console.warn('[periodic Supabase sync] failed:',err?.message||err);}},15000);return()=>clearInterval(iv);},[loaded]);
