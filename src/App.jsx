...
function PorteurDashboard({soc,reps,allM,socBank,ghlData,setPTab}){
 ...
 const kpis=[
  {label:"CA du mois",value:`${fmt(ca)}€`,trend:caTrend,accent:C.acc},
 ...
 return <div className="fu">
  ...
  <div className="fade-up" style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:14,padding:20,marginBottom:20,animationDelay:"0.4s"}}>
   ...
   <svg viewBox="0 0 480 200" style={{width:"100%",height:"auto"}}>
    {chartData.map((d,i)=>{
     ...
     return <g key={i}>
      <rect x={x} y={180-hCa} width={bw} height={hCa} fill={C.acc} rx={4} style={{transformOrigin:`${x+bw/2}px 180px`,animation:`barGrow 0.6s ease ${0.1*i}s both`}}/>
      ...
      {d.ca>0&&<text x={x+bw/2} y={180-hCa-4} textAnchor="middle" fill={C.acc} fontSize="8" fontWeight="700" fontFamily={FONT}>{fK(d.ca)}</text>}
     </g>;
    })}
    <line x1="30" y1="180" x2="470" y2="180" stroke={C.brd} strokeWidth="1"/>
   </svg>
   <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:8}}>
    <div style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:C.td}}><span style={{width:10,height:10,borderRadius:3,background:C.acc}}/> CA</div>
  ...
  {recentTx.length>0&&<div className="fade-up" style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:14,padding:16,marginBottom:20,animationDelay:"0.8s"}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
    ...
    <span onClick={()=>setPTab(5)} style={{color:C.acc,fontSize:10,fontWeight:600,cursor:"pointer"}}>Voir tout →</span>
   </div>
  ...
  {ghlStats&&<div className="fade-up" style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:14,padding:16,animationDelay:"0.9s"}}>
   ...
   <div style={{display:"flex",gap:12,marginBottom:12}}>
    <div><div style={{fontSize:20,fontWeight:900,color:C.acc}}>{fmt(ghlStats.pipelineValue)}€</div><div style={{fontSize:9,color:C.td}}>Valeur pipeline</div></div>
