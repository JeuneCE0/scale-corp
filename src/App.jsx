...
/* ===== SALES PANEL ===== */
function SalesPanel({soc,ghlData,socBankData,clients,reps,setPTab}){
...
/* ===== PUBLICITE PANEL ===== */
function PublicitePanel({soc,ghlData,socBankData,clients,reps,setPTab}){
 const cm=curM();
...
function Sidebar({items,activeTab,setTab,brandTitle,brandSub,onLogout,onTour,extra,dataTourPrefix,brand,onThemeToggle,installPrompt,adminBack}){
...
/* 5. WAR ROOM */
function AdminClientsTab({clients,socs}){
...
function WarRoom({soc,reps,allM,ghlData,clients,socBank,socs,onClose,readOnly}){
...
/* 6. AUTO-PILOT MODE */
function AutoPilotSection({soc,clients,ghlData,socBank,reps}){
...
function SynergiesAutoPanel({socs,reps,clients,ghlData}){
 const cm=curM();
...
/* 8. WIDGET EMBED */
function WidgetEmbed({soc,clients}){
...
function WidgetCard({soc,clientCount,theme}){
...
function WidgetRenderer({socId,socs,clients}){
...
setLErr("Code incorrect");setShake(true);setTimeout(()=>setShake(false),500);},[pin,socs,onboarded]);
 ...
 return <div className="glass-bg" style={{display:"flex",minHeight:"100vh",fontFamily:FONT,color:C.t}}>
  ...
  <div className="mobile-header" style={{display:"none",position:"fixed",top:0,left:0,right:0,zIndex:100,background:"rgba(14,14,22,.8)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"10px 16px",alignItems:"center",gap:10}}>
   ...
   <span style={{fontWeight:800,fontSize:14,fontFamily:FONT_TITLE,color:C.acc}}>⚡ {hold.brand?.name||"L'INCUBATEUR ECS"}</span>
  </div>
