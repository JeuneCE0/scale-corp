...
export default function Affiliation() {
  ...
  const referralLink = `hubscale.app/r/${org.slug || data.referralCode}`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
      ...
      {/* ── Stats row: Clicks, Leads, Sales, Earnings ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1.3fr', gap: 12 }}>
        ...
        <div className="glass-static" style={{ padding: '16px 18px', borderRadius: 12, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <Btn small v="secondary" onClick={requestPayout} style={{ fontSize: 11, padding: '4px 10px' }}>
      ...
      {/* ── Tab Content ── */}
      {activeTab === 0 && <QuickstartTab referralLink={referralLink} onCopy={copyLink} copied={copied} onRequestPayout={requestPayout} pendingPayout={stats.pendingPayout} />}
      ...
      {activeTab === 2 && <LinksTab referralLink={referralLink} code={data.referralCode} onCopy={copyLink} copied={copied} />}
...
// ---------------------------------------------------------------------------
// Quickstart Tab (like Tella screenshot: Share your link + Receive earnings)
// ---------------------------------------------------------------------------

function QuickstartTab({ referralLink, onCopy, copied, onRequestPayout, pendingPayout }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
      ...
      <div className="glass-static" style={{
        padding: '28px 24px', borderRadius: 14, textAlign: 'center',
        border: `1px solid ${T.green}25`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16,
      }}>
        ...
        <div style={{ position: 'relative', marginBottom: 4 }}>
          ...
          <div style={{
            ...
            fontSize: 14, boxShadow: `0 2px 8px ${T.green}40`,
          }}>
            ✓
          </div>
        </div>
        <div>
          ...
          <div style={{ fontSize: 12, color: T.textSecondary, lineHeight: 1.5, maxWidth: 280 }}>
            {t('affiliation.receiveEarningsDesc')}
          </div>
        </div>
        <Btn v="secondary" onClick={onRequestPayout} style={{ width: '100%', justifyContent: 'center' }}>
          {t('affiliation.connectPayouts')}
        </Btn>
      </div>
    </div>
  );
...
function LinksTab({ referralLink, code, onCopy, copied }) {
        ...
        {leaderboard.map((entry, i) => (
          <div key={entry.rank} style={{
            ...
            <div style={{
              ...
              fontSize: i < 3 ? 18 : 13, fontWeight: 700, color: i < 3 ? T.accent : T.textMuted,
            }}>
            ...
            <div style={{ fontSize: 12, color: T.textSecondary, minWidth: 90, textAlign: 'right' }}>
              {entry.referrals} {t('affiliation.referralsLabel')}
            </div>
            <div style={{ fontSize: 13, fontWeight: 700, color: T.green, minWidth: 80, textAlign: 'right' }}>
              {fmt(entry.earned)}€
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {faqs.map((faq, i) => (
          <div key={i}>
            ...
            {openIdx === i && (
              <div style={{ padding: '12px 16px 16px', fontSize: 13, color: T.textSecondary, lineHeight: 1.6 }}>
                {faq.a}
              </div>
            )}
          </div>
...
function ShareModal({ link, code, onClose }) {
