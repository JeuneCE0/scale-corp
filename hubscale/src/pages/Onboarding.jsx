...
export default function Onboarding() {
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      {/* Step indicators */}
      <div className="fade-up d1" style={{ display: 'flex', gap: 4, marginBottom: 24, justifyContent: 'center', flexWrap: 'wrap' }}>
        {STEPS.map((s) => (
            ...
            {TOOLS.map((t) => {
              ...
              return (
                <div key={t.id} onClick={() => toggleTool(t.id)} style={{
                  ...
                  {sel && <span style={{ marginLeft: 'auto', color: T.accent, fontWeight: 700, fontSize: 14 }}>✓</span>}
                </div>
              );
            })}
          </div>
      ...
      {step === 3 && (
        <Card className="fade-up d2">
          ...
          <div style={{ fontSize: 11, color: T.textSecondary, marginBottom: 16 }}>
            Renseignez vos clés API pour connecter vos services. Vous pourrez les modifier plus tard.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            <Inp label="Stripe Secret Key" value={apiKeys.stripe} onChange={(v) => setApiKeys({ ...apiKeys, stripe: v })} placeholder="sk_live_..." note="Trouvable dans Dashboard Stripe → Développeurs → Clés API" />
            <Inp label="Revolut API Key" value={apiKeys.revolut} onChange={(v) => setApiKeys({ ...apiKeys, revolut: v })} placeholder="prod-..." note="Trouvable dans Revolut Business → API" />
            <Inp label="GoHighLevel API Key" value={apiKeys.ghl} onChange={(v) => setApiKeys({ ...apiKeys, ghl: v })} placeholder="eyJ..." note="Trouvable dans Settings → API Keys" />
            <Inp label="Meta Ads Access Token" value={apiKeys.meta} onChange={(v) => setApiKeys({ ...apiKeys, meta: v })} placeholder="EAA..." note="Trouvable dans Meta Business Suite → Paramètres" />
          </div>
          <div style={{ marginTop: 12, padding: 12, borderRadius: 8, background: T.orangeBg, border: `1px solid ${T.orange}22` }}>
            <div style={{ fontSize: 11, color: T.orange }}>
              🔒 Vos clés sont chiffrées et stockées de manière sécurisée. Vous pouvez passer cette étape et les ajouter plus tard.
            </div>
          </div>
        </Card>
      )}
      ...
      {step === 4 && (
        <Card className="fade-up d2">
          ...
          <div style={{ fontSize: 11, color: T.textSecondary, marginBottom: 16 }}>
            Comment souhaitez-vous alimenter vos données financières ?
          </div>
      ...
      {step === 5 && (
        <Card className="fade-up d2">
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ fontSize: 13, color: T.textSecondary, marginBottom: 20, maxWidth: 400, margin: '0 auto 20px' }}>
              Votre espace client est configuré. Découvrez les fonctionnalités principales de HubScale.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10, maxWidth: 500, margin: '0 auto' }}>
              {[
                { icon: '📊', title: 'Dashboard', desc: 'Vue d\'ensemble de votre activité' },
                { icon: '👥', title: 'CRM', desc: 'Gérez vos contacts et pipeline' },
                { icon: '💰', title: 'Data', desc: 'Suivez vos finances' },
                { icon: '🔬', title: 'Data Health', desc: 'Vérifiez vos intégrations' },
              ].map((f) => (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20 }}>
        <div>
          {step > 1 && <Btn v="ghost" onClick={prev}>← Précédent</Btn>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {step < 5 ? (
            ...
            <Btn style={{ background: 'linear-gradient(135deg, #f97316, #f59e0b)' }}>Accéder au Dashboard →</Btn>
          )}
        </div>
      </div>
    </div>
