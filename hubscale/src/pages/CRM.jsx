...
export default function CRM() {
  ...
  const editableLimit = canAccessPro() ? Infinity : isPaid() ? 100 : 20;
  const manualCount = contacts.filter((c) => !c.source).length;
  const atContactLimit = manualCount >= editableLimit;
  ...
  const handleCSVImport = useCallback((e) => {
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        newContacts.push({ id: uid(), name, email, company, phone, status, notes: '', commentaires: [], history: [], createdAt: new Date().toISOString(), source: 'csv_import' });
        imported++;
      }
